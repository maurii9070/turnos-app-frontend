...
const patientCreateSchema = v.pipe(
  v.object({
    dni: v.pipe(
...
type PatientCreateOutput = v.InferOutput<typeof patientCreateSchema>
...
const state = reactive<Partial<PatientCreateOutput>>({
  dni: '',
  firstName: '',
  lastName: '',
  dateOfBirth: '',
...
async function onSubmit(event: FormSubmitEvent<PatientCreateOutput>) {
  loading.value = true
  try {
    ...
    const message = error?.data?.message || error?.message || 'Error al registrar el paciente'
  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="mt-1 text-muted">
        Creá una cuenta para un nuevo paciente.
      </p>
    </div>
